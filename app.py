...
st.caption("目標: 80kg → 70kg（週3ジムで【筋トレマシン限定 ＋ 有酸素】効率ダイエット）")
    ...
    WEEKLY_SCHEDULE = {
        0: { # 月曜日
            ...
            "menu": [
                ...
                "3. **ディップスマシン（二の腕裏・胸下部）**: **12回 × 3セット** （休憩 60秒）\n   - 【マシン】シートに座り、両脇のグリップを下に押し下げる",
...
with tab3:
    ...
    st.write("「マシンのシートの高さはどう合わせる？」「このマシンがジムにない」など、何でも相談してください！")
    
    user_query = st.text_area("質問・相談を入力", placeholder="例：チェストプレスで肩が痛くならないシート設定のコツを教えて！")
    ...
    if st.button("ジェミに相談する 💬", use_container_width=True):
        if user_query and api_key:
            ...
            system_prompt = (
                ...
                "ユーザーのお使いのジムでは『ダンベル・バーベルなどのフリーウェイト』『ケーブルマシン』『自重トレ』は一切使わず、『筋トレマシン（ウエイトマシン）のみ』でトレーニングを行います。"
                "必ず筋トレマシンのみを用いた代替案や設定アドバイスを提供し、親切かつモチベーションが上がる回答をしてください。"
            )
